...
/// クライアント（見守られ側）に後からメール+パスワードを付与する画面（任意）。
///
/// 目的: 機種変更・再インストール時に、同じ見守り関係を新端末へ引き継ぐ。
/// 成功したら `Navigator.pop(true)` で呼び出し元に登録完了を通知する。
class ClientEmailScreen extends ConsumerStatefulWidget {
  const ClientEmailScreen({super.key});

  @override
  ConsumerState<ClientEmailScreen> createState() => _ClientEmailScreenState();
}

class _ClientEmailScreenState extends ConsumerState<ClientEmailScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await ref.read(apiClientProvider).registerClientEmail(
            clientToken: token,
            email: email,
            password: password,
          );
